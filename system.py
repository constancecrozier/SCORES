  ...
  #      curt = use[2]/n_years
   ...
   #         f.write(str(cycles)+' cycles per year\n\n')
    ...
    def fully_optimise(self,fossilLimit,fixed_capacities=False):
        ...
        == returns ==
        '''
        #Extend all the plugin timetables to be sufficiently long for the optimisation
        for i in range(0,self.aggEV_list.n_assets):            
            # Check that the input timeseries are divisible by 24
            if not self.aggEV_list.assets[i].Nin.size % 24 == 0:
                print('Nin/Nout data for fleet ' + self.aggEV_list.assets[i].name + ' is not exactly divisible by 24hrs, could lead to unnatural periodicities.')
            
            #increase the length of the in/out plugin series to be longer than the simulation
            repeat_num = self.len // self.aggEV_list.assets[i].Nin.size
            self.aggEV_list.assets[i].Nin = np.tile(self.aggEV_list.assets[i].Nin,repeat_num+1)
            repeat_num = self.len // self.aggEV_list.assets[i].Nout.size
            self.aggEV_list.assets[i].Nout = np.tile(self.aggEV_list.assets[i].Nout,repeat_num+1)
    ...
    def plot_timeseries(self,start=0,end=-1):
        ...
        if (self.aggEV_list.Pfos.shape == ()):
            ...
            for g in range(len(self.gen_list)):
                ax.plot(range(start,timehorizon), self.gen_list[g].power_out[start:timehorizon], color='tab:red', label='Gen '+self.gen_list[g].name)
